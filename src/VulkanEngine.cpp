...
static VkShaderModule CreateShaderModule(VulkanEngine* VulkanEngine, const std::vector<char>& code)
{
	...
	if (VulkanEngine->GetDispatchTable().createShaderModule(&createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
...
void VulkanEngine::LoadMeshes()
{
	...
	m_TriangleMesh.Vertices[0].Color = { 0.f, 1.f, 0.0f };
	m_TriangleMesh.Vertices[1].Color = { 0.f, 1.f, 0.0f }; 
	m_TriangleMesh.Vertices[2].Color = { 0.f, 1.f, 0.0f }; 
...
void VulkanEngine::CreateCommandBuffers()
{
	...
	for (size_t i = 0; i < m_RenderData->CommandBuffers.size(); i++) {
		...
		m_DispatchTable.cmdBindVertexBuffers(m_RenderData->CommandBuffers[i], 0, 1, &m_TriangleMesh.VertexBuffer.Buffer, &offset);

		m_DispatchTable.cmdDraw(m_RenderData->CommandBuffers[i], m_TriangleMesh.Vertices.size(), 1, 0, 0);
...
void VulkanEngine::InitDevice()
{
	if (!m_Window)
	{
		printf("VulkanEngine holds no reference to a window.");
		return;
	}
...
void VulkanEngine::CreateSurfaceGLFW(VkAllocationCallbacks* allocator /* = nullptr */)
{
...
vkb::DispatchTable& VulkanEngine::GetDispatchTable()
{
